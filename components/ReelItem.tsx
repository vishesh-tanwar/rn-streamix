...
const ReelItem = ({ video, isActive }: { video: Video; isActive: boolean }) => {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {paused && (
          ...
          >
            <MaterialIcons name="play-arrow" color="orange" size={40} />
          </View>
        )}
        ...
        <View style={styles.overlay}>
          <View className="pb-2 flex-1 flex-row items-center">
            ...
            <View>
              <Text className="text-white ml-2">{video.userName}</Text>
            </View>
          </View>
          <Text style={styles.title}>{video.title}</Text>
          <Pressable onPress={() => setShowDescription(!showDiscription)}>
            <Text style={styles.desc} numberOfLines={showDiscription ? 0 : 2}>
              {video.description}
            </Text>
          </Pressable>
        </View>
